...
fn draw_frame(engine: &Engine, runtime_data: &mut Pipeline) {
	unsafe {
		...
		let render_pass_begin_info = vk::RenderPassBeginInfo::default().render_pass(runtime_data.renderpass).framebuffer(runtime_data.framebuffers[present_index as usize]).render_area(engine.swapchain.surface_resolution.into()).clear_values(&clear_values);
		...
		record_submit_commandbuffer(
			...
			|device, draw_command_buffer| {
				...
				runtime_data.vertex_shader.vertex_buffer.bind(engine, current_frame);
				runtime_data.vertex_shader.index_buffer.bind(engine, current_frame);
		...
		engine.swapchain.present(engine, current_frame, present_index);
